...
export default function Trade() {
  ...
  async function createTradeRequest() {
    ...
    try {
        const transaction = await swapContract.makeTradeRequest(receiverAddress, Number(senderTokenId), Number(receiverTokenId))
  ...
  async function approveSenderToken() {
    if ( !account ) {
      ...
      try {
        ...
        const transaction = await nftContract.approve(process.env.NEXT_PUBLIC_SWAP_ADDRESS, senderTokenId)
        await transaction.wait()
    } catch(error) {
        console.log(error)
    }
    ...
    <div className="max-w-xl mt-36 mx-auto text-center px-4">
      ...
      <main className="space-y-8 mt-2">
            ...
            <div className="space-y-8">
                <div className="grid gap-6 w-full md:grid-cols-2">
                    <div className="w-full relative inline-flex items-center justify-center">
                        <button
                          ...
                          onClick={() => {setFormMode(1); setShowTradeDetails(false); setShowInvalidQuery(false)}}
                          disabled={!account}
                        >
                          Query Trade
                        </button>
                    ...
                    <div className="w-full relative inline-flex items-center justify-center">
                        <button
                          className="bg-blue-600 hover:bg-blue-700 text-white py-4 px-8 rounded-md w-full"
                          onClick={() => {setFormMode(2); setShowTradeDetails(false); setShowInvalidQuery(false); setSenderAddress(account)}}
                          disabled={!account}
                        >
                          Create Trade
                        </button>
                ...
                {formMode == 2 && <div className="flex flex-col space-y-4">
                <input onChange={(event) => {
                    setReceiverAddress(event.target.value)
                  }} type="text" placeholder="Receiver Address" className="border p-4 w-100 text-center" />
                  <input onChange={(event) => {
                    setSenderTokenId(event.target.value)
                  }} type="number" placeholder="Sender Token Id" className="border p-4 w-100 text-center" />
                  <input onChange={(event) => {
                    setReceiverTokenId(event.target.value)
                  }} type="number" placeholder="Receiver Token Id" className="border p-4 w-100 text-center" />
                  {senderStatus == 2 && <button
                      className="bg-blue-600 hover:bg-blue-700 text-white py-4 px-8 rounded-md w-full"
                      onClick={() => approveSenderToken()}
                    >
                      Approve Token To Trade
                  </button>}
                  {senderStatus == 3 && <button 
                    className="bg-blue-600 hover:bg-blue-700 text-white py-4 px-8 rounded-md w-full"
                ...
                {formMode == 1 && <div className="flex flex-col space-y-4">
                <input onChange={(event) => {
                    setPendingTradeId(event.target.value)
                    /*setTradeId(event.target.value)*/
                  }} type="number" placeholder="Trade ID" className="border p-4 w-100 text-center" />
                  <button
                    className="bg-blue-600 hover:bg-blue-700 text-white py-4 px-8 rounded-md w-full"
                ...
                {showInvalidQuery && <div className="flex flex-col space-y-4">That Trade ID is invalid</div>}

                {showTradeDetails && <div className="flex flex-col space-y-4">
                    <div>
                        ...
                        <div>Is Trade Open: <b>{isTradeOpen.toString()}</b></div>
                    </div>
                    {isTradeOpen && <div className="flex flex-col space-y-4">
                        {account.toLowerCase() == receiverAddress.toLowerCase() && <div className="flex flex-col space-y-4">
                            <button
                      className="bg-blue-600 hover:bg-blue-700 text-white py-4 px-8 rounded-md w-full"
                      onClick={rejectTrade}
                    >
                      Reject Trade Request
                            ...
                            {receiverStatus == 3 && <button
                      className="bg-blue-600 hover:bg-blue-700 text-white py-4 px-8 rounded-md w-full"
                      onClick={acceptTrade}
                    >
                      Accept Trade Request
                  </button>} 
                  {receiverStatus == 2 && <button
                      className="bg-blue-600 hover:bg-blue-700 text-white py-4 px-8 rounded-md w-full"
