...
contract Swap is Ownable {
    ...
    function makeTradeRequest(
        address _receiver,
        uint256 _senderTokenId,
        uint256 _receiverTokenId
    ) public {
        require(
            IERC721(nftContractAddress).ownerOf(_senderTokenId) == msg.sender,
            "Sender must own offered NFT"
        );
        require(
            IERC721(nftContractAddress).ownerOf(_receiverTokenId) == _receiver,
            "Receiver must own requested NFT"
        );
        require(
            IERC721(nftContractAddress).getApproved(_senderTokenId) ==
                address(this),
    ...
    function acceptTradeRequest(uint256 _tradeId) public {
        ...
        require(
            IERC721(nftContractAddress).ownerOf(trade.senderTokenId) ==
                trade.sender,
            "Sender must own offered NFT"
        ); // Redundant
        require(
            IERC721(nftContractAddress).ownerOf(trade.receiverTokenId) ==
                msg.sender,
            "Receiver must own requested NFT"
        ); // Redundant
        require(
            IERC721(nftContractAddress).getApproved(trade.senderTokenId) ==
                address(this),
            "Sender must approve this contract"
        ); // Redundant
        require(
            IERC721(nftContractAddress).getApproved(trade.receiverTokenId) ==
                address(this),
    ...
    function queryTradeRequest(uint256 _tradeId)
        public
        view
        returns (
            address,
    ...
    {
        require(
            _tradeId <= tradeId.current(),
            "Querying a trade that doesn't exist"
        );
        Trade memory trade = trades[_tradeId];
        return (
            trade.sender,
