...
describe("Swap", function () {
  ...
  describe("Successful Trades", function () {
    it("Should complete open trade", async function () {
      const { swap, testNFT, owner, otherAccount, anotherAccount } = await loadFixture(
        deploySwapAndNFTFixture
      );
      ...
      await expect(swap.makeTradeRequest(otherAccount.address, 0, 1))
        .to.emit(swap, "TradeCreated")
        .withArgs(0, owner.address, otherAccount.address, 0, 1);
      await expect(swap.connect(otherAccount).acceptTradeRequest(0))
        .to.emit(swap, "TradeUpdated")
        .withArgs(0, "accepted");
    });
    ...
    it("Sender can cancel open trade", async function () {
      ...
      await expect(swap.makeTradeRequest(otherAccount.address, 0, 1))
        .to.emit(swap, "TradeCreated")
        .withArgs(0, owner.address, otherAccount.address, 0, 1);
      await expect(swap.cancelTradeRequest(0))
        .to.emit(swap, "TradeUpdated")
        .withArgs(0, "canceled");
    });
    ...
    it("Receiver can reject open trade", async function () {
      ...
      await expect(swap.makeTradeRequest(otherAccount.address, 0, 1))
        .to.emit(swap, "TradeCreated")
        .withArgs(0, owner.address, otherAccount.address, 0, 1);
      await expect(swap.connect(otherAccount).rejectTradeRequest(0))
        .to.emit(swap, "TradeUpdated")
        .withArgs(0, "rejected");
    });
    ...
    it("Receiver can reject open trade after transferring own NFT", async function () {
      ...
      await testNFT
        ...
        .transferFrom(otherAccount.address, anotherAccount.address, 1);
  ...
  describe("Invalid Trade Actions", function () {
    it("Revert open trade if sender doesn't own nft", async function () {
      ...
      await expect(swap.makeTradeRequest(otherAccount.address, 2, 1)).to.be.revertedWith(
        "Sender must own offered NFT"
      ); // TODO: Remove the with statement after removing redundant contract checks
    });
    ...
    it("Revert open trade if receiver doesn't own nft", async function () {
      ...
      await expect(swap.makeTradeRequest(otherAccount.address, 0, 2)).to.be.revertedWith(
        "Receiver must own requested NFT"
      ); // TODO: Remove the with statement after removing redundant contract checks
    });
    ...
    it("Revert open trade if sender hasn't approved contract", async function () {
      ...
      await expect(swap.makeTradeRequest(otherAccount.address, 1, 2)).to.be.revertedWith(
        "Sender must approve this contract"
      ); // TODO: Remove the with statement after removing redundant contract checks
    });
    ...
    it("Revert accept trade when receiver hasn't approved contract", async function () {
      ...
      await expect(swap.connect(otherAccount).acceptTradeRequest(0)).to.be.revertedWith(
        "Receiver must approve this contract"
      );
    });
    ...
    it("Revert when sender has revoked token approval", async function () {
      ...
      await expect(swap.connect(otherAccount).acceptTradeRequest(0)).to.be.revertedWith(
        "Sender must approve this contract"
      ); // TODO: Remove with statement after removing redundant contract lines
    });
    ...
    it("Revert when trying to reject an already rejected trade", async function () {
      ...
      await expect(swap.connect(otherAccount).rejectTradeRequest(0))
        .to.emit(swap, "TradeUpdated")
        .withArgs(0, "rejected");
      await expect(swap.connect(otherAccount).rejectTradeRequest(0)).to.be.revertedWith(
        "Trade is not open"
      );
    });
    ...
    it("Revert when trying to accept trade as sender", async function () {
      ...
      await expect(swap.makeTradeRequest(otherAccount.address, 0, 1))
        .to.emit(swap, "TradeCreated")
        .withArgs(0, owner.address, otherAccount.address, 0, 1);
      await expect(swap.acceptTradeRequest(0)).to.be.revertedWith(
        "Only trade receiver can accept trade"
      );
    });
    ...
    it("Revert when trying to accept trade as other account", async function () {
      ...
      await expect(swap.makeTradeRequest(otherAccount.address, 0, 1))
        .to.emit(swap, "TradeCreated")
        .withArgs(0, owner.address, otherAccount.address, 0, 1);
      await expect(swap.connect(anotherAccount).acceptTradeRequest(0)).to.be.revertedWith(
        "Only trade receiver can accept trade"
      );
    });
    ...
    it("Revert when trying to reject trade as sender", async function () {
      ...
      await expect(swap.makeTradeRequest(otherAccount.address, 0, 1))
        .to.emit(swap, "TradeCreated")
        .withArgs(0, owner.address, otherAccount.address, 0, 1);
      await expect(swap.rejectTradeRequest(0)).to.be.revertedWith(
        "Only trade receiver can reject trade"
      );
    });
    ...
    it("Revert when trying to reject trade as other account", async function () {
      ...
      await expect(swap.makeTradeRequest(otherAccount.address, 0, 1))
        .to.emit(swap, "TradeCreated")
        .withArgs(0, owner.address, otherAccount.address, 0, 1);
      await expect(swap.connect(anotherAccount).rejectTradeRequest(0)).to.be.revertedWith(
        "Only trade receiver can reject trade"
      );
    });
    ...
    it("Revert when trying to cancel trade as receiver", async function () {
      ...
      await expect(swap.makeTradeRequest(otherAccount.address, 0, 1))
        .to.emit(swap, "TradeCreated")
        .withArgs(0, owner.address, otherAccount.address, 0, 1);
      await expect(swap.connect(otherAccount).cancelTradeRequest(0)).to.be.revertedWith(
        "Only trade requester can cancel trade"
      );
    });
    ...
    it("Revert when trying to reject trade as other account", async function () {
      ...
      await expect(swap.makeTradeRequest(otherAccount.address, 0, 1))
        .to.emit(swap, "TradeCreated")
        .withArgs(0, owner.address, otherAccount.address, 0, 1);
      await expect(swap.connect(anotherAccount).cancelTradeRequest(0)).to.be.revertedWith(
        "Only trade requester can cancel trade"
      );
    });
  });
  ...
  describe("Querying Trades", function () {
    ...
    it("Revert when querying a trade that doesn't exist", async function () {
      ...
      await expect(swap.makeTradeRequest(otherAccount.address, 0, 1))
        .to.emit(swap, "TradeCreated")
        .withArgs(1, owner.address, otherAccount.address, 0, 1);
